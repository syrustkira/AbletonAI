...
class PlatformPackagingTests(unittest.TestCase):
 ...
 def test_reconnect_rejection_is_atomic(self):
  session=PluginSession(ProtocolVersion(1,2),{"audio"});handshake=PluginHandshake(ProtocolVersion(1,0),{"audio"},"song-a","workspace-a")
  self.assertEqual(session.negotiate(handshake),{"audio"});self.assertEqual(session.reconnect(handshake),{"audio"})
  for rejected in (PluginHandshake(ProtocolVersion(1,0),{"audio"},"song-b","workspace-a"),PluginHandshake(ProtocolVersion(1,0),{"audio"},"song-a","workspace-b")):
   with self.assertRaises(PermissionError):session.reconnect(rejected)
   self.assertEqual((session.song_id,session.workspace_id,session.state),("song-a","workspace-a",CoreConnectionState.READY))
  self.assertEqual(session.reconnect(PluginHandshake(ProtocolVersion(2,0),{"audio"},"song-a","workspace-a")),set())
  self.assertEqual((session.song_id,session.workspace_id,session.state),("song-a","workspace-a",CoreConnectionState.READY))
 def test_unspecified_compatibility_never_authorizes_mutation(self):
  descriptor=HostCapabilityDescriptor("write",True,IntegrationTier.DEEP,ComponentState.READY,"new-adapter")
  self.assertEqual(descriptor.compatibility,CompatibilityState.NEEDS_REVALIDATION);self.assertFalse(descriptor.eligible(OperationRisk.MUTATE));self.assertFalse(descriptor.eligible(OperationRisk.READ))
  descriptor.compatibility=CompatibilityState.ASSUMED_COMPATIBLE;self.assertTrue(descriptor.eligible(OperationRisk.READ));self.assertFalse(descriptor.eligible(OperationRisk.MUTATE))
 def test_daw_mutation_contract_requires_gate1(self):
  verified=HostCapabilityDescriptor("midi_modify",True,IntegrationTier.DEEP,ComponentState.READY,"ableton",compatibility=CompatibilityState.VERIFIED)
 ...
 def test_deep_capability_failure_does_not_downgrade_siblings_or_maturity(self):
  caps={x:HostCapabilityDescriptor(x,True,IntegrationTier.DEEP,ComponentState.READY,"ableton",compatibility=CompatibilityState.VERIFIED) for x in ("transport_read","automation_read","automation_write","midi_read","midi_modify","routing_read","routing_set")}
 ...
 def test_job_combines_healthy_automatic_and_failed_guided_manual(self):
  caps={"track_read":HostCapabilityDescriptor("track_read",True,IntegrationTier.DEEP,ComponentState.READY,"ableton",compatibility=CompatibilityState.VERIFIED),"device_read":HostCapabilityDescriptor("device_read",True,IntegrationTier.DEEP,ComponentState.READY,"ableton",compatibility=CompatibilityState.VERIFIED),"automation_write":HostCapabilityDescriptor("automation_write",True,IntegrationTier.DEEP,ComponentState.DEGRADED,"ableton",reason="write unavailable",fallback_candidates=["plugin_bridge"],compatibility=CompatibilityState.VERIFIED)}
 ...
 def test_subcapability_failures_and_recovery_are_isolated(self):
  ...
  adapter=HostAdapterDescriptor("Ableton",IntegrationTier.DEEP,capabilities={name:HostCapabilityDescriptor(name,True,IntegrationTier.DEEP,ComponentState.READY,"ableton",compatibility=CompatibilityState.VERIFIED) for name in names},song_id="song-1")
 ...
 def test_resolution_replaces_only_failed_capability(self):
  deep={name:HostCapabilityDescriptor(name,True,IntegrationTier.DEEP,ComponentState.READY,"ableton",compatibility=CompatibilityState.VERIFIED) for name in ("track_read","automation_write")}
  deep["automation_write"].runtime_state=ComponentState.DEGRADED
  bridge={"automation_write":HostCapabilityDescriptor("automation_write",True,IntegrationTier.GENERIC,ComponentState.READY,"plugin_bridge",compatibility=CompatibilityState.VERIFIED)}
 ...
 def test_host_update_changes_only_named_compatibility(self):
  caps={name:HostCapabilityDescriptor(name,True,IntegrationTier.DEEP,ComponentState.READY,"ableton",host_version="12.3",compatibility=CompatibilityState.VERIFIED) for name in ("transport_read","midi_read","automation_write","routing_set_output")}
 ...
 def test_adapter_update_revalidates_only_affected_capabilities(self):
  caps={name:HostCapabilityDescriptor(name,True,IntegrationTier.DEEP,ComponentState.READY,"ableton",adapter_version="1.8.3",compatibility=CompatibilityState.VERIFIED) for name in ("transport_read","midi_read","automation_write","clip_envelope_modify")}
  adapter=HostAdapterDescriptor("Ableton",IntegrationTier.DEEP,capabilities=caps,song_id="song")
 ...
 def test_capability_circuit_breaker_does_not_quarantine_adapter(self):
  caps={name:HostCapabilityDescriptor(name,True,IntegrationTier.DEEP,ComponentState.READY,"ableton",compatibility=CompatibilityState.VERIFIED) for name in ("transport_read","automation_write")};adapter=HostAdapterDescriptor("Ableton",IntegrationTier.DEEP,capabilities=caps)
