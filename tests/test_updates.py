...
class UpdateTests(unittest.TestCase):
 ...
 def test_offline_pauses_without_fetch(self):
  with tempfile.TemporaryDirectory() as td:
   _,v,_=self.fixture(td);called=[];e=UpdateEngine(Path(td),NetworkPolicy(NetworkMode.OFFLINE),v,"mac","arm64",{})
 ...
 def test_component_only_plan_and_pending_host_close(self):
  with tempfile.TemporaryDirectory() as td:
   data,v,m=self.fixture(td,host_close=True);e=UpdateEngine(Path(td),NetworkPolicy(NetworkMode.FULL),v,"mac","arm64",{"CORE":"2.4.0","ABLETON_ADAPTER":"1.8.3"},song_id="song")
 ...
 def test_hash_signature_platform_architecture_fail_closed(self):
  with tempfile.TemporaryDirectory() as td:
   data,v,m=self.fixture(td);e=UpdateEngine(Path(td),NetworkPolicy(),v,"mac","arm64",{"CORE":"2.4.0"})
 ...
 def test_offline_import_and_rollback_preserve_identity_and_policy(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   policies={"ai":"OFF","network":"OFFLINE","community":"OFF","privacy":"PRIVATE"};e=UpdateEngine(root/"state",NetworkPolicy(NetworkMode.OFFLINE),v,"mac","arm64",{"CORE":"2.4.0","ABLETON_ADAPTER":"1.8.3"},policies=policies,song_id="song")
 ...
 def test_migration_and_dependencies_fail_closed(self):
  with tempfile.TemporaryDirectory() as td:
   _,v,m=self.fixture(td);m.components[0].migration_version="arbitrary-code";m.signature=v.sign(manifest_bytes(m));e=UpdateEngine(Path(td),NetworkPolicy(),v,"mac","arm64",{"CORE":"2.4.0"})
   with self.assertRaises(ValueError):e.validate_manifest(m)
