...
Compatibility trust is operation-sensitive. Read and observation jobs may use `ASSUMED_COMPATIBLE` implementations when runtime state is healthy and no contrary evidence exists. Automatic mutations require `VERIFIED` compatibility as well as Gate 1 authority. An assumed mutation path may be promoted only by an explicitly non-mutating, capability-specific probe; otherwise resolution selects another verified implementation or Guided Manual.
