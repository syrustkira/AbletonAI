...
void n0te_gain_process(n0te_gain*s,float*x,size_t n){if(s->bypass)return;for(size_t i=0;i<n;i++)x[i]=clamp(x[i]*s->gain,-1,1);}
...
void n0te_transient_process(n0te_transient*s,float*x,size_t n){if(s->bypass)return;for(size_t i=0;i<n;i++){float peak=fabsf(x[i]);s->fast=s->fast_coeff*s->fast+(1-s->fast_coeff)*peak;s->slow=s->slow_coeff*s->slow+(1-s->slow_coeff)*peak;float transient=fmaxf(0,s->fast-s->slow)/fmaxf(s->fast,1e-9f);float gain=fmaxf(0,1+s->attack*transient+s->sustain*(1-transient)*.5f);x[i]=clamp(x[i]*gain,-1,1);}}
