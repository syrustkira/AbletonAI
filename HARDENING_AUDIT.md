...
4. **Server integration coverage — improved, still a continuing quality target:** 71 tests cover scoping, same-process unrelated-Set refusal, stable-ID recovery after index shifts, transaction chronology, recursive targeting, atomic concurrent writers, standalone doctor state, proposal TTL, malformed/oversized bodies, 404/409/503 classifications, local Host/Origin rejection, and ambiguous simplification recovery without native Undo. Metadata-only rotating diagnostics are implemented.
