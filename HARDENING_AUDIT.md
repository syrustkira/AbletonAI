...
These findings were discovered after the original 1.2.4 installer audit and are intentionally recorded as **open**, not silently treated as fixed:

1. **User Library diagnostic gap:** the installer can consistently use a remembered/chosen User Library, but the first real-Mac acceptance run showed that the chosen path can still differ from the User Library Live is actually configured to use. Once the setting was corrected, `Ableton_Live_MCP` appeared and N0TE detected refreshed Live state. Next release should add a Remote Script Doctor that distinguishes installed files, Live load evidence and likely path mismatch.
2. **Transaction ownership / cross-Set Undo risk:** current transaction lookup/recovery needs explicit song/Set ownership and affected-target post-state validation. Automatic recovery must fail closed when ownership cannot be proven. This is P0 before real-song mutation use.
3. **Threaded-state hardening:** the companion uses a threaded HTTP server while several JSON state writes and locks are not yet consistently atomic/stable. This should be hardened before MCP or additional concurrent callers.
4. **Server integration coverage:** the 53-test suite passes, but the server/orchestration layer remains the main coverage gap. Reliability tests should target state conflicts, failure semantics, concurrent mutation prevention and standalone health/uninstall paths.
