...
4. **Server integration coverage — improved, still a continuing quality target:** 59 tests cover scoping, cross-Set refusal, recursive targeting, atomic concurrent writers and standalone doctor state. Typed failures, local request policy, body caps, proposal TTL and metadata-only rotating diagnostics are implemented.
